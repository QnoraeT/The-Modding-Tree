var systemComponents = {
	...
	'overlay-head': {
		...
		<span v-if="player.points.log10().pow(1.25).gte((tmp.reductionFactors ?? { sc3: { start: D(Infinity), eff: D(1) } }).sc3.start.log10()) && (tmp.reductionFactors ?? { sc3: { exp: D(1) } }).sc3.exp.neq_tolerance(1)" class="overlayThing">Your points past {{ format((tmp.reductionFactors ?? { sc3: { start: D(Infinity), eff: D(1) } }).sc3.start) }} are being slowed down by /{{ format((tmp.reductionFactors ?? { sc3: { start: D(Infinity), eff: D(1) } }).sc3.eff, 2) }}!<br></span>
