var systemComponents = {
	...
	'tree-node': {
      ...
      v-if="tmp[layer].tooltip != ''"
			:text="(tmp[layer].isLayer) ? (
				player[layer].unlocked ? (tmp[layer].tooltip ? tmp[layer].tooltip : format(player[layer].points, 0) + ' ' + tmp[layer].resource)
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : 'Reach ' + format(tmp[layer].requires, 0) + ' ' + tmp[layer].baseResource + ' to unlock (You have ' + format(tmp[layer].baseAmount, 0) + ' ' + tmp[layer].baseResource + ')')
			)
			: (
	...
	'overlay-head': {
		...
		<span v-if="player.points.pow(1.25).gte((tmp.reductionFactors ?? { sc1: { start: D(Infinity), eff: D(1) } }).sc1.start) && (tmp.reductionFactors ?? { sc1: { exp: D(1) } }).sc1.exp.neq_tolerance(1)" class="overlayThing">Your points past {{ format((tmp.reductionFactors ?? { sc1: { start: D(Infinity), eff: D(1) } }).sc1.start) }} are being slowed down by /{{ format((tmp.reductionFactors ?? { sc1: { start: D(Infinity), eff: D(1) } }).sc1.eff, 2) }}!<br></span>
		<span v-if="player.points.pow(1.25).gte((tmp.reductionFactors ?? { sc2: { start: D(Infinity), eff: D(1) } }).sc2.start) && (tmp.reductionFactors ?? { sc2: { exp: D(1) } }).sc2.exp.neq_tolerance(1)" class="overlayThing">Your points past {{ format((tmp.reductionFactors ?? { sc2: { start: D(Infinity), eff: D(1) } }).sc2.start) }} are being slowed down by /{{ format((tmp.reductionFactors ?? { sc2: { start: D(Infinity), eff: D(1) } }).sc2.eff, 2) }}!<br></span>
		<span v-if="player.points.log10().pow(1.25).gte((tmp.reductionFactors ?? { sc3: { start: D(Infinity), eff: D(1) } }).sc3.start.log10()) && (tmp.reductionFactors ?? { sc3: { exp: D(1) } }).sc3.exp.neq_tolerance(1)" class="overlayThing">Your points past {{ format((tmp.reductionFactors ?? { sc3: { start: D(Infinity), eff: D(1) } }).sc3.start) }} are being slowed down by ^1/{{ format((tmp.reductionFactors ?? { sc3: { start: D(Infinity), eff: D(1) } }).sc3.eff, 2) }}!<br></span>
