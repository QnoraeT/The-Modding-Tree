...
// data = a function returning the content (actually HTML)
	...
	Vue.component('main-display', {
		props: ['layer', 'data'],
		template: `
		<div><span v-if="player[layer].points.lt('1e1000')">You have </span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}">{{data ? format(player[layer].points, data) : format(player[layer].points, 0)}}</h2> {{tmp[layer].resource}}<span v-if="layers[layer].effectDescription">, <span v-html="run(layers[layer].effectDescription, layers[layer])"></span></span><br><br></div>
		`
	})
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			<span v-if="tmp[layer].baseAmount"><br>You have {{format(tmp[layer].baseAmount, 0)}} {{tmp[layer].baseResource}}</span>
			...
			<br><br>
			<span v-if="tmp[layer].showBest">Your best {{tmp[layer].resource}} is {{format(player[layer].best, 0)}}<br></span>
			<span v-if="tmp[layer].showTotal">You have made a total of {{format(player[layer].total, 0)}} {{tmp[layer].resource}}<br></span>
		</div>
		`
	})
